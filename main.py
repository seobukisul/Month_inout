...
def get_latest_report():
    ...
    try:
        response = requests.get(url, verify=False) # In case of SSL issues, but normally True is better
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # Select all rows in the board
        ...
        for article in articles:
            ...
            # Check if the title matches "O월" and "수출입 동향"
            if f"{target_month}월" in title and "수출입" in title and "동향" in title:
                ...
                # Find PDF file download link
                # Note: This is an estimation, class names might vary.
                # Usually there are links to attached files.
                ...
                # If the above fails, let's search for any link ending in .pdf
...
def analyze_pdf(pdf_path):
    ...
    2. 시각화를 위해 주요 품목별 수출 증감률(%) 데이터를 JSON 형식으로 추출해줘. 
    JSON 형식: {{"summary": "요약 내용", "data": {{"반도체": 15.2, "자동차": -3.1, "철강": 2.5}}}}
    데이터가 있는 품목만 포함해줘.
...
def create_graph(data_dict):
    # GitHub Actions ubuntu runner might not have Malgun Gothic, so use a default sans-serif
    plt.rcParams['font.family'] = 'sans-serif'
    # Fallback to a font that supports Korean in ubuntu, like NanumGothic if installed. 
    # For now, let's just use default and handle English names or ensure Nanum is installed in workflow if needed.
